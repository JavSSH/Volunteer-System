...
def userProfileTableSetup():
    conn = sqlite3.connect("database/volunteer_system.db")
...
def getUser(email, password):
    ...
    conn.row_factory = sqlite3.Row
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM user as u WHERE email = ? AND password = ?", (email, password))
...
if __name__ == "__main__":
    ...
    print(getUser("brander.dobbins@volunteer.com", "password123")) # UserAdmin
